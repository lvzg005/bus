// 校验
Activator.activeValidation("role_menu_create_form",
// 校验字段
{
	role_name : "required"
...
$(document).ready(function(){	
	...
	$("button[name='role_create_submit']").click(function(){
		var arr = new Array();
		...
		$("form[id='role_menu_create_form']").submit();
	});
	...
	$("select").each(function(i){
		...
		if(i == 1){
			//初始化一级菜单数据
			var lv1Data = $.grep(setMenus, function(o){
				return o.lv == '1';
			});
			...
			selectComp.each(function(){
				$(this).change(function(){					
					...
					var option = $(this);
					var lv2Data = $.grep(setMenus, function(o){
					...
					$($("select").get(2)).each(function(){
						$(this).change(function(){
							$($("select").get(3)).find("option").remove();
							var option = $(this);
							var lv3Data = $.grep(setMenus, function(o){
