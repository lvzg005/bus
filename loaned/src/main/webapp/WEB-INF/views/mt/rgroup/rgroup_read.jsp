<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ include file="../../common/common.jsp"%>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>角色组管理</title>
</head>
<body>
	<jsp:include page="../../common/header.jsp" />
	<pack:script src="rgroup_read.js" />
	<div class="container" style="width: 90%">
		<fieldset>
			<div id="legend">
				<legend>角色组管理</legend>
			</div>
			<form id="exForm" class="form-search" method="post" action="rgroup_read.html">
				<input type="hidden" name="opt" value="s5001" />
				<input type="hidden" name="subBtn" />
				<input type="button" id="addSubmit" class="btn" value="新增" /> 
				<input type="button" id="editSubmit" class="btn" value="编辑" /> 
				<input type="button" id="delSubmit" class="btn" value="删除" />
				<table id="example">
					<thead>
						<tr>
							<th>角色ID</th>
							<th>组ID</th>
							<th>角色组名称</th>
							<th>角色类型</th>
							<th>角色名称</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="roleGroup" items="${pageList.list}">
							<tr>
								<td><c:out value="${roleGroup.roleId}" /></td>
								<td><c:out value="${roleGroup.groupId}" /></td>
								<td><c:out value="${roleGroup.groupName}" /></td>
								<td><c:out value="${roleGroup.roleType}" /></td>
								<td><c:out value="${roleGroup.roleName}" /></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
				<!-- 配合分页参数使用 -->
				<div id="exPageData">
					<input type="hidden" value="${pageList.fullListSize}" /> <input
						name="pageSize" type="hidden" value="${pageList.objectsPerPage}" /> <input name="page"
						type="hidden"
						value='<c:choose><c:when test="${pageList.pageNumber > 0}">${pageList.pageNumber}</c:when><c:otherwise>1</c:otherwise></c:choose>' />
				</div>
				<!-- 配合搜索栏使用 -->
				<div id="exSearchData">
					<input name="search_key" type="hidden" value='${_search_key}' /> <input
						name="search_value" type="hidden" value='${_search_value}' />
				</div>
			</form>
		</fieldset>
	</div>
	<jsp:include page="../../common/footer.jsp" />
</body>
</html>
