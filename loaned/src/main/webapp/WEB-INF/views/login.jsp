...
<%@page import="com.company.bus.loaned.biz.shiro.ShiroDbRealm.ShiroUser"%>
<%@page import="com.company.bus.loaned.dbaccess.entity.SetUser"%>
...
<body>
	<shiro:authenticated>
		...
		<h3 style="text-align:center;margin-top:10%;">
			<span class="text-success">用户[<span class="text-error"><%=((ShiroUser)SecurityUtils.getSubject().getPrincipal()).getUserName()%></span>]已登录</span>
		</h3>
		<p align="center" style="margin-top:5%">
			<button type="button" class="btn" onclick="javascript:location.href='<%=pageContext.getServletContext().getContextPath()%>/default.html'">返回首页</button>
		</p>
	</shiro:authenticated>
