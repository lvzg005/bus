...
!function( $ ) {
	...
	var Datepicker = function(element, options){
		...
		//开启/关闭清空按钮 - add by caobin
		if(options.enableClear){
			$(this.element).append('<button class="btn" type="button"><i class="icon-remove"></i></button>');
			$(this.element).find("button").click($.proxy(function(){
	...
	Datepicker.prototype = {
		...
		setValue: function() {
			...
			this.element.find('input').blur();//add by caobin, 触发校验
		},
	...
	//style="z-index: 1200;" add by caobin - 于modal前显示，bootstrap默认modal的z-index=1050
	DPGlobal.template = '<div class="datepicker dropdown-menu" style="z-index: 1200;">'+
