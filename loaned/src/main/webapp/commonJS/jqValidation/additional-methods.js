...
/** 日期范围校验-created by caobin **/
jQuery.validator.addMethod("dateRange", function(value, element, param){	
	var target = $(param);
	var endDate = target.val();
	if (this.settings.onfocusout) {
		target.unbind(".validate-dateRange").bind("blur.validate-dateRange", function() {	
			$(element).valid();			
		});
	}
	if(value && endDate){
		var regex = /\D/ig;
		return value.replace(regex, "") - 0 <= endDate.replace(regex, "");
	}
	return true;
}, jQuery.validator.messages.dateRange);

/** 金额范围校验-created by caobin **/
jQuery.validator.addMethod("amtRange", function(value, element, param){	
	var target = $(param);
	var eAmt = target.val();
	if (this.settings.onfocusout) {
		target.unbind(".validate-amtRange").bind("blur.validate-amtRange", function() {	
			$(element).valid();			
		});
	}
	if(value && eAmt){
	...
	return true;
}, jQuery.validator.messages.amtRange);

//扩展jquery校验金额规则
jQuery.validator.addMethod("minAmount", function(value, element, param) {
	return this.optional(element) || value > param;
},jQuery.format("请输入大于零的金额值"));

/** 日期范围校验-created by caobin **/
jQuery.validator.addMethod("dateRange30", function(value, element, param){	
	var target = $(param);
	var endDate = target.val();
	if (this.settings.onfocusout) {
		target.unbind(".validate-dateRange30").bind("blur.validate-dateRange30", function() {	
			$(element).valid();			
		});
	}
	if(value && endDate){
		var regex = /\D/ig;
		var start=new Date(value.replace(regex, "/"));
		var end = new Date(endDate.replace(regex, "/"));
		return  end-start <= 30*24*3600*1000;
	}
	return true;
}, jQuery.validator.messages.dateRange30);

/** 校验商户号是否存在 - created by why **/
jQuery.validator.addMethod("cusidexists", function(value, element, param){
	var target = $(param);
//	var endDate = target.val();
//	if (this.settings.onfocusout) {
//		target.unbind(".validate-dateRange30").bind("blur.validate-dateRange30", function() {	
...
//	}
	if (this.settings.onfocusout) {
		target.unbind(".validate-cusidexists").bind("blur.validate-cusidexists", function() {	
			$(element).valid();			
		});
	}
	var flagsss = true;
	if(value){
	 $.ajax({
		 type: "post",
		 url:$("#contextpath").val()+"account/validateuser.html",
		 async:false,
		 data:{cusId:value},
		 datatype:'text',
		 complete: function(d){
			var data = $.evalJSON(d.responseText);
			var flag = data.flag;
			if(flag==='success'){
				flagsss=true;
//				bootbox.dialog('录入成功',[{
...
//					"class" : "btn-primary",
//					"callback": function() {
//						window.location.href=$('#form').attr('action')+'?opt='+$('#opt').val();
//					}
//				}]);
			}else{
				flagsss=false;
//				bootbox.confirm('录入失败');
//				window.location.href=$('#form').attr('action')+'?opt='+$('#opt').val();
			}
		 }});
	}else{
	...
	return flagsss;
}, jQuery.validator.messages.cusidexists);

jQuery.validator.addMethod("cusacctexists", function(value, element, param){
	var target = $(param);
	if (this.settings.onfocusout) {
		target.unbind(".validate-cusacctexists").bind("blur.validate-cusacctexists", function() {	
			$(element).valid();			
		});
	}
	var flagsss = true;
	if(value){
	 $.ajax({
		 type: "post",
		 url:$("#contextpath").val()+"account/validateacc.html",
		 async:false,
		 data:{cusAcct:value},
		 datatype:'text',
		 complete: function(d){
...
jQuery.validator.addMethod("is30acc", function(value, element, param){
	var target = $(param);
	if (this.settings.onfocusout) {
		target.unbind(".validate-is30acc").bind("blur.validate-is30acc", function() {	
			$(element).valid();			
		});
	}
	var flagsss = true;
	...
	return flagsss;
}, jQuery.validator.messages.is30acc);
